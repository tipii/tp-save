...
export default function DeliveredLivraisonsSection() {
  const trpc = useTRPC();

  const { data: deliveredLivraisons, isLoading } = useQuery(
  ...
  return (
    <Card>
      ...
      <CardContent className="max-h-[300px] overflow-y-auto">
        <div className="space-y-3">
          {deliveredLivraisons?.map((livraison: any) => (
            <div
              key={livraison.id}
          ...
          {(!deliveredLivraisons || deliveredLivraisons.length === 0) && (
            <div className="py-6 text-center text-gray-500">
              <CheckCircle className="mx-auto mb-2 h-8 w-8 text-gray-300" />
              <p>Aucune livraison effectuée récemment</p>
            </div>
          )}
        </div>
      </CardContent>
